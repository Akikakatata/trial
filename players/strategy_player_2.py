...
class StrategicPlayer(Player):
    ...
    def update_possible_positions_after_hit(self, ship_name, hit_coords):
        # Update the possible positions for the ship after a successful hit
        possible_positions = []
        for position in self.possible_positions[ship_name]:
            if self.is_within_attack_range(position, hit_coords):
    ...
    def action(self):
        if self.prev_attack_coords is not None:
            ...
            if self.prev_attack_coords[0] == "attack" and self.prev_attack_coords[1] == "hit":
                ...
                self.update_possible_positions_after_hit(ship_hit, self.prev_attack_coords[2])
    ...
    def select_attack_coordinate(self, possible_positions):
        ...
        # If no successful attack, select a random coordinate
